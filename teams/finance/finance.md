# Finance & Payments Department — Business Review

## About This Department

The Finance & Payments team manages Swiggy's revenue streams, payment processing,
partner and rider payouts, refund disbursals, and financial health. Every order on the
platform flows through this team — from the customer's UPI tap to the restaurant's
settlement and the rider's earnings credit.

---

## Financial Year: **2023**

### Revenue & Profitability

| Metric                                     | Value |
| ------------------------------------------ | ----- |
| Gross order value (₹ Cr)                   |  10   |
| Platform commission revenue (₹ Cr)         |   2   |
| Delivery fee revenue (₹ Cr)                |   3   |
| Subscription revenue — Swiggy Pro (₹ Cr)   |   4   |
| Total gross revenue (₹ Cr)                 |  19   |
| Net revenue after payouts (₹ Cr)           |  16   |

### Payment Mode Split

| Payment Mode               | % of Transactions | Avg Transaction Value |
| -------------------------- | ----------------- | --------------------- |
| UPI                        |         80        |         950           |
| Credit / Debit Card        |         10        |        1500           |
| Wallet (Paytm / PhonePe)   |          5        |         500           |
| Cash on Delivery (COD)     |          2.5      |         500           |
| Swiggy Credits / Coupons   |          2.5      |         150           |

### Payouts & Settlements

| Metric                          | Value |
| ------------------------------- | ----- |
| Total restaurant payouts (₹ Cr) |   17  |
| Total rider payouts (₹ Cr)      |    3  |
| Average settlement cycle (days) |    4  |
| Payout disputes raised          |   70  |

### Refunds & Losses

| Metric                              |  Value  |
| ----------------------------------- | ------- |
| Total refunds disbursed (₹ Cr)      |    0.47  |
| Refund rate (% of orders)           |    3    |
| Fraud / policy-abuse cases detected |  700    |
| Estimated fraud loss (₹ Lakhs)      |    0.7  |

---

## Financial Year: **2024**

### Revenue & Profitability

| Metric                                     | Value |
| ------------------------------------------ | ----- |
| Gross order value (₹ Cr)                   |  50   |
| Platform commission revenue (₹ Cr)         |   5   |
| Delivery fee revenue (₹ Cr)                |  10   |
| Subscription revenue — Swiggy Pro (₹ Cr)   |   2   |
| Total gross revenue (₹ Cr)                 |  20   |
| Net revenue after payouts (₹ Cr)           |  10   |

### Payment Mode Split

| Payment Mode               | % of Transactions | Avg Transaction Value |
| -------------------------- | ----------------- | --------------------- |
| UPI                        |       60          |           50          |
| Credit / Debit Card        |       20          |          250          |
| Wallet (Paytm / PhonePe)   |        6          |          100          |
| Cash on Delivery (COD)     |        5          |          300          |
| Swiggy Credits / Coupons   |        9          |          100          |

### Payouts & Settlements

| Metric                          | Value |
| ------------------------------- | ----- |
| Total restaurant payouts (₹ Cr) |  60   |
| Total rider payouts (₹ Cr)      |  10   |
| Average settlement cycle (days) |  30   |
| Payout disputes raised          |  10   |

### Refunds & Losses

| Metric                              | Value |
| ----------------------------------- | ----- |
| Total refunds disbursed (₹ Cr)      |   6   |
| Refund rate (% of orders)           |  88   |
| Fraud / policy-abuse cases detected |   5   |
| Estimated fraud loss (₹ Lakhs)      | 1.2   |

---
## Financial Year: **2025**

### Revenue & Profitability

| Metric                                     | Value |
| ------------------------------------------ | ----- |
| Gross order value (₹ Cr)                   |   20   |
| Platform commission revenue (₹ Cr)         |   10   |
| Delivery fee revenue (₹ Cr)                |   120  |
| Subscription revenue — Swiggy Pro (₹ Cr)   |    40  |
| Total gross revenue (₹ Cr)                 |   30   |
| Net revenue after payouts (₹ Cr)           |   20   |

### Payment Mode Split

| Payment Mode               | % of Transactions | Avg Transaction Value |
| -------------------------- | ----------------- | --------------------- |
| UPI                        |         4         |          2             |
| Credit / Debit Card        |         3         |          1             |
| Wallet (Paytm / PhonePe)   |         2         |          3             |
| Cash on Delivery (COD)     |         4         |          6             |
| Swiggy Credits / Coupons |           1         |          44            |

### Payouts & Settlements

| Metric                          | Value |
| ------------------------------- | ----- |
| Total restaurant payouts (₹ Cr) |  1    |
| Total rider payouts (₹ Cr)      |  2    |
| Average settlement cycle (days) |  3    |
| Payout disputes raised          |  4    |

### Refunds & Losses

| Metric                              | Value |
| ----------------------------------- | ----- |
| Total refunds disbursed (₹ Cr)      |   1   |
| Refund rate (% of orders)           |   5   |
| Fraud / policy-abuse cases detected |   2   |
| Estimated fraud loss (₹ Lakhs)      |   3   |

## Financial Year: **2026**

### Revenue & Profitability

| Metric                                     | Value |
| ------------------------------------------ | ----- |
| Gross order value (₹ Cr)                   |    7  |
| Platform commission revenue (₹ Cr)         |    3  |
| Delivery fee revenue (₹ Cr)                |    5  |
| Subscription revenue — Swiggy Pro (₹ Cr)	 |    4  |
| Total gross revenue (₹ Cr)                 |    3  |
| Net revenue after payouts (₹ Cr)           |    2  |

### Payment Mode Split

| Payment Mode               | % of Transactions | Avg Transaction Value |
| -------------------------- | ----------------- | --------------------- |
| UPI                        |       10          |      300              |
| Credit / Debit Card        |       20          |      300              |
| Wallet (Paytm / PhonePe)   |       25          |      200              |
| Cash on Delivery (COD)     |       20          |      100              |
| Swiggy Credits / Coupons   |       20          |      100              |

### Payouts & Settlements

| Metric                          | Value |
| ------------------------------- | ----- |
| Total restaurant payouts (₹ Cr) |   5   |
| Total rider payouts (₹ Cr)      |   4   |
| Average settlement cycle (days) |   3   |
| Payout disputes raised          |   2   |

### Refunds & Losses

| Metric                              | Value |
| ----------------------------------- | ----- |
| Total refunds disbursed (₹ Cr)      |   22  |
| Refund rate (% of orders)           |   55  |
| Fraud / policy-abuse cases detected |   1   |
| Estimated fraud loss (₹ Lakhs)      |   56  |